...
async function upperThresholdHandle(
  ...
  const currencyConfig = await rawdb.findOneCurrency(manager, internalRecord.currency);
  if (!currencyConfig) {
...
async function lowerThresholdHandle(manager: EntityManager, sentRecord: WithdrawalTx) {
  ...
  const currencyConfig = await rawdb.findOneCurrency(manager, sentRecord.currency);
